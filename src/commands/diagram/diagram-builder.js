...
const { SQSClient, ListQueueTagsCommand, GetQueueUrlCommand } = require("@aws-sdk/client-sqs");
const { KinesisClient, ListTagsForStreamCommand } = require("@aws-sdk/client-kinesis");
...
const describeMap = {
  ...
  lambda: {
    func: (item) => lambda.send(new ListTagsCommand({ Resource: item.Arn })),
    tags: (item) =>
  ...
  states: {
    func: (item) =>
      stepFunctions.send(new sf.ListTagsForResourceCommand({ resourceArn: item.Arn })),
    tags: (item) => item.tags,
  },
  sns: {
    func: (item) =>
      sns.send(new sn.ListTagsForResourceCommand({ ResourceArn: item.Arn })),
    tags: (item) =>
      item.Tags.map((p) => {
        return { key: p.Key, value: p.Value };
      }),
  ...
  sqs: {
    func: (item) => {
      ...
      return sqs.send(new GetQueueUrlCommand({ QueueName: queueName }))
        .then((url) => {
          return sqs.send(new ListQueueTagsCommand({ QueueUrl: url.QueueUrl }));
        });
    },
    tags: (item) =>
      Object.keys(item.Tags).map((p) => {
